...
for i,j in zip(dir2, dir4):
    ...
    parser.add_argument('--data', type=str, default=j, help='data')
    ...
    def run_command(command):
        output, error = subprocess.Popen(
            command, shell=True, executable='/bin/bash', stdout=subprocess.PIPE, stderr=subprocess.PIPE
        ).communicate()
        return output.decode('utf-8'), error.decode('utf-8')
    
    def get_gpu_info():
        gpu_info, _ = run_command('nvidia-smi --query-gpu=memory.total,memory.used --format=csv,nounits,noheader')
        gpu_info = gpu_info.strip().split('\n')
        gpu_info = [i.split(',') for i in gpu_info]
        gpu_info = {f'GPU {i}': {'total_memory': int(j[0]), 'used_memory': int(j[1])} for i, j in enumerate(gpu_info)}
        return gpu_info
    ...
    for ii in range(args.itr):
        ...
        setting = '{}_ft{}_sl{}_ll{}_pl{}_dm{}_nh{}_el{}_dl{}_df{}_at{}_fc{}_eb{}_dt{}_mx{}_{}_{}'.format(args.model,
                                                                                                             args.features,
                                                                                                             args.seq_len,
                                                                                                             args.label_len,
                                                                                                             args.pred_len,
        ...
        print("The gpu occupied by this prediction is as follows(unit:B):")
        print(size)
